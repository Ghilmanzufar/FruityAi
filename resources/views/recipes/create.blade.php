...
<div class="bg-slate-50 min-h-screen py-10">
    <div class="max-w-4xl mx-auto px-4">
        ...
        <div class="text-center mb-10">
            <h1 class="text-3xl font-bold text-slate-800">Tambah Resep Baru 🍳</h1>
            <p class="text-slate-500">Isi detail masakan untuk melengkapi database AI.</p>
        </div>

        <div class="bg-white rounded-3xl shadow-lg border border-slate-100 overflow-hidden">
            <form action="{{ route('recipes.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
                @csrf
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-2">
                            ...
                            <input type="text" name="title" required placeholder="Contoh: Jus Alpukat Kocok" 
                        ...
                        <div>
                            ...
                            <select name="main_ingredient" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all outline-none">
                                ...
                                <option value="avocado">Avocado (Alpukat)</option>
                            ...
                            <p class="text-xs text-slate-400 mt-1">*Pilih sesuai label yang dikenali AI</p>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Foto Masakan</label>
                        ...
                        <div>
                            ...
                            <input type="number" name="cooking_time" required placeholder="15" 
                                   class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all outline-none">
                        </div>
                        <div>
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea name="description" rows="3" required placeholder="Jelaskan rasa dan keunikan masakan ini..." 
                                      class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all outline-none"></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div id="ingredient-list" class="space-y-3">
                        <div class="flex gap-2">
                            <input type="text" name="ingredients[]" required placeholder="Contoh: 2 buah Pisang Ambon" 
                                   class="flex-1 rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all outline-none">
                            <button type="button" disabled class="text-slate-300 px-3 cursor-not-allowed"><i class="fa-solid fa-trash"></i></button>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div id="step-list" class="space-y-3">
                        <div class="flex gap-2 items-start">
                            <span class="mt-3 text-emerald-600 font-bold text-sm w-8 text-center bg-emerald-50 py-1 rounded-md border border-emerald-100">1</span>
                            <textarea name="steps[]" required rows="2" placeholder="Contoh: Kupas pisang dan potong kecil-kecil." 
                                      class="flex-1 rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all outline-none"></textarea>
                            <button type="button" disabled class="text-slate-300 px-3 mt-3 cursor-not-allowed"><i class="fa-solid fa-trash"></i></button>
                        </div>
                    </div>
                </div>

                <div class="pt-6 border-t border-slate-100">
                    <button type="submit" class="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold py-4 rounded-xl shadow-lg transition-transform transform hover:-translate-y-1">
                        Simpan Resep Baru 💾
                    </button>
                </div>
...
<script>
    // Style Class yang sama untuk elemen baru (biar konsisten tebalnya)
    ...
    // 1. Fungsi Tambah Bahan
    function addIngredient() {
        const container = document.getElementById('ingredient-list');
        const div = document.createElement('div');
        div.className = 'flex gap-2 animate-fade-in'; // Tambah animasi dikit
        div.innerHTML = `
    ...
    // 2. Fungsi Tambah Langkah
    function addStep() {
        const container = document.getElementById('step-list');
        const count = container.children.length + 1; // Hitung nomor urut
        const div = document.createElement('div');
        div.className = 'flex gap-2 items-start animate-fade-in';
        div.innerHTML = `
            <span class="mt-3 text-emerald-600 font-bold text-sm w-8 text-center bg-emerald-50 py-1 rounded-md border border-emerald-100">${count}</span>
